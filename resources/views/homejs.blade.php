  ...
  (function(window, document, $) {
...

      {
        ...
        plugins: [
          ...
          Chartist.plugins.fillDonut({
            items: [{
              content: '<p class="small">Solde</p><h5 class="mt-0 mb-0"> 10k</h5>'
            }]
          })
        ]
      }
    ...
    var TotalTransactionLine = new Chartist.Line(
      "#total-transaction-line-chart", {
        series: [
          [3, 10, 4, 20, 7, 45, 5, 35, 20, 48, 30, 100]
        ]
      }, {
        chartPadding: 0,
        axisX: {
    ...
    var UserStatisticsBarChart = new Chartist.Bar(
      ...
      "#user-statistics-bar-chart", {
        ...
        series: [
          [4000, 8000, 12000, 14000, 12000],
          [5000, 10000, 13000, 12000, 11000]
        ]
      }, {
    ...
    $.ajax({
      ...
      success: function(data) {
        if (data.success == true) {
          $('#turnoverofmonth').html(data.thisMonTurnOver)
          months = [];
          sumTotal = [];
          for (var key in data.orders) {
            months.push(key)
            sumTotal.push(data.orders[key])
          }
          var SLlabels = months;
          ...
          gradientStroke.addColorStop(0, "#8EC741")
          gradientStroke.addColorStop(1, "#F4681E")

          var gradientFill = LineSL3ctx.createLinearGradient(500, 0, 0, 200)
          gradientFill.addColorStop(0, "#8EC741")
          gradientFill.addColorStop(1, "#F4681E")
    ...
    $.ajax({
      ...
      success: function(data) {
        if (data.success == true) {
          // console.log(data)
          $('#orders').html(data.thisMonthOrders)
          months = [];
          sumTotal = [];
          for (var key in data.orders) {
            months.push(key)
